...
\end{tikzpicture}}
  \caption{Above the barrier, the input path, entirely derivable from the
  sequence. Below it, the supervision. \texttt{pdbe-arpeggio} reads the 3D
  structure and its output crosses the barrier at a single point, the training
  loss; at inference the lower half is absent. The three icons come from the same
